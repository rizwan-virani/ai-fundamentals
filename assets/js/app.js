/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `AIFUND` namespace with
...
(function () {
  ...
  var S = window.AIFUND || {};
  var PASS = (S.exam && S.exam.passing) || 80;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("aifund." + k, fb); },
      set: function (k, v) { write("aifund." + k, v); },
      del: function (k) { try { localStorage.removeItem("aifund." + k); } catch (e) {} }
    };
  })();
  ...
  function statCard() {
    ...
    return el("div.kpis", null, [
      ...
      kpi(mocks.length ? best : "—", "Best mock (%)", best >= PASS ? "good" : (mocks.length ? "warn" : "")),
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the course is structured and scored, and where AI Fundamentals fits in your studies and career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "How the course is structured, assessed, and scored &mdash; the modules, the practice mock, and the 80% mastery target, so you know what you are working toward.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where AI Fundamentals fits: a foundational, field-agnostic AI-literacy credential, the prompter / verifier / editor roles it supports, and how it prepares you for AI-shaped work.",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every key AI Fundamentals term and acronym across all six domains in one deck &mdash; the fastest way to lock in the vocabulary.",
    ...
    /* ---- 04 · Practice Quizzes ---- */
    var b4 = dashSection("05", "Practice quizzes", (S.questions ? S.questions.length : "600") + " questions · 4 modes");
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "600") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all six." }));
    ...
    var b5 = dashSection("08", "Full-length practice mock", "80 questions · timed");
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length practice assessment &mdash; 80 questions across the six domains. Sit it under timed conditions to benchmark your readiness against the 80% mastery target." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic 100-minute practice assessment: 80 questions across the six domains, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard.",
      foot: "80 questions · 100 minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official CompTIA AI Fundamentals course page, Elements of AI, prompting guides, and the NIST AI Risk Management Framework.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.AIFUND_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. AI Fundamentals" });
  ...
  function openLabSandbox(lab, i) {
    ...
    function runCommand(raw) {
      ...
      if (low === "whoami") { logLine("student (AI Fundamentals learner)"); return; }
  ...
  function computeReadiness() {
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? Math.round(bestMock) : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "%") : "not attempted" },
  ...
  function readinessOutcome(R) {
    if (R.bestMock != null) {
      var m = R.bestMock;
      if (m >= PASS + 6) return { cls: "good", text: "Strong mastery. Your best mock of " + m + "% clears the " + PASS + "% mastery target with margin." };
      if (m >= PASS) return { cls: "good", text: "On track. Your best mock of " + m + "% is at or above the " + PASS + "% mastery target. Keep it consistent." };
      if (m >= PASS - 8) return { cls: "warn", text: "Borderline. Your best mock of " + m + "% sits just under the " + PASS + "% target. A focused push should get you there." };
      return { cls: "crit", text: "Not yet. Your best mock of " + m + "% is below the mastery target. Keep building before you move on." };
    }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates AIFUND.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.AIFUND_VER ? "?v=" + window.AIFUND_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "Domain " + id + " · " + d.weight + "% of this course", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Course mechanics", "Course Mechanics: CompTIA AI Fundamentals", "How the course is structured, assessed, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where AI Fundamentals fits in your path", S.careerGuidance || [])); }
      ...
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var scoreCell = r.mode === "mock"
            ? el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.scaled + "%" })
