/* ============================================================================
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating AIFUND.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (aifund.flash.v1) keyed by card id,
...
(function () {
  ...
  var DECK_SIZE = 100;
  var DOMS = (S.domainMeta || []).map(function (d) { return d.id; }); if (!DOMS.length) DOMS = [1, 2, 3, 4, 5, 6];
  ...
  function ver() { return window.AIFUND_VER ? "?v=" + window.AIFUND_VER : ""; }
  ...
  window.FLASH = {
    ...
    analytics: function () {
      ...
      var byD = {}; for (var _di = 0; _di < DOMS.length; _di++) { var n = DOMS[_di]; byD[n] = { total: DECK_SIZE, studied: 0, mastered: 0, due: this.domainDue(n) }; }
      for (var id in st) {
