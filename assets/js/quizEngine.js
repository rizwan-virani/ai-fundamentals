...
AIFUND.questions = AIFUND.questions || [];
AIFUND.pbqs = AIFUND.pbqs || [];

/* Question + PBQ DATA live in content/quiz1-6.js and content/pbqs.js (loaded before this file). */
...
(function () {
  ...
  /* ===========================================================================
     ...
     Persists two things across every quiz/mock session (localStorage via APP):
       aifund.objstats.v1 = { "1.2": {c, n}, ... }   per-objective accuracy
       aifund.missed.v1   = { "D4-071": 1, ... }      questions answered wrong
  ...
  function startMock() {
    ...
    A.open({
      tag: "Mock exam", render: function (host) {
        ...
        host.appendChild(el("div.phead", null, [el("h1", { text: "Full-Length Mock Exam" }), el("span.sub", { text: S.exam.maxQuestions + " questions · across the six domains" })]));
    ...
    function submit() {
      ...
      var pct = Math.round(correct / items.length * 100);
      var scaled = pct; // percentage (0-100); this course has no scaled score
      QS.record(items, picks); // feed adaptive engine + missed-question queue
